...
describe('withOnyxTest', () => {
    ...
    it('should batch correctly together little khachapuris', async () => {
        ...
        Onyx.merge(ONYX_KEYS.SIMPLE_KEY_2, 'prev_string2');
    ...
    it.skip('should update withOnyx subscriber just once when mergeCollection is used', async () => {
        ...
        Onyx.mergeCollection(ONYX_KEYS.COLLECTION.TEST_KEY, {test_1: {ID: 123}, test_2: {ID: 234}, test_3: {ID: 345}} as GenericCollection);
    ...
    it('should update withOnyx subscribing to individual key if mergeCollection is used', async () => {
    ...
    it('should update withOnyx subscribing to individual key with merged value if mergeCollection is used', async () => {
    ...
    it('should pass a prop from one connected component to another', async () => {
        const onRender = jest.fn();
        const markReadyForHydration = jest.fn();
        ...
        const TestComponentWithOnyx = withOnyx<ViewWithCollectionsProps, Record<'staticObject' | 'dependentObject' | 'multiDependentObject' | 'extremeMultiDependentObject', unknown>>({
            staticObject: {
                key: `${ONYX_KEYS.COLLECTION.STATIC}1`,
            },
            dependentObject: {
                key: ({staticObject}) => `${ONYX_KEYS.COLLECTION.DEPENDS_ON_STATIC}${(staticObject && typeof staticObject === 'object' && 'id' in staticObject && staticObject.id) || 0}`,
            },
            multiDependentObject: {
                key: ({dependentObject}) =>
                    `${ONYX_KEYS.COLLECTION.DEPENDS_ON_DEPENDS_ON_STATIC}${(dependentObject && typeof dependentObject === 'object' && 'id' in dependentObject && dependentObject.id) || 0}`,
            },
    ...
    it('mergeCollection should merge previous props correctly to the new state', async () => {
        const onRender = jest.fn();
        const markReadyForHydration = jest.fn();
    ...
    it('merge with a simple value should not update a connected component unless the data has changed', async () => {
        const onRender = jest.fn();
    ...
    it('should cache missing keys', async () => {
        const TestComponentWithOnyx = withOnyx<ViewWithTextProps, ViewWithTextOnyxProps>({
            text: {
                key: ONYX_KEYS.TEST_KEY,
            },
    ...
    it('should cache empty collections', async () => {
        const TestComponentWithOnyx = withOnyx<ViewWithCollectionsProps, {text: unknown}>({
            text: {
                key: ONYX_KEYS.COLLECTION.TEST_KEY,
            },
        ...
        await StorageMock.setItem(ONYX_KEYS.SIMPLE_KEY, 'simple');
    ...
    it.skip('works with selectors', async () => {
        ...
        sourceData = null;
        Onyx.set(ONYX_KEYS.TEST_KEY, sourceData);
        await act(async () => waitForPromisesToResolve());

        // Correct data has been passed to the selector
        expect(selector).not.toHaveBeenCalledWith('world', expect.anything());
        expect(selector).not.toHaveBeenCalledWith('dougal', expect.anything());
        expect(selector).toHaveBeenLastCalledWith(undefined, {text: 'world'});
